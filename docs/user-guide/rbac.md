...
- seven **permission flags**: `can_create_template`, `can_manage_users`,
  `can_manage_group_instances`, `can_manage_docker`, `can_manage_registry`,
  `can_view_monitoring`, `can_view_audit_logs`;
...
|---|---|---|---|---|
| **Admin** | `admin` | all seven **TRUE**, fixed | unlimited (NULL) | cannot be renamed or deleted; flags fixed; ceiling editable by an admin |
| **Manager** | `manager` | all seven **TRUE** on seed, editable | 2 | cannot be renamed or deleted; flags editable by an admin; ceiling editable |
| **User** | `user` | all seven **FALSE**, fixed | 1 | cannot be renamed or deleted; flags fixed; ceiling editable |
...
| Action | Guardrail |
|---|---|
| Delete a user | actor's tier must be **strictly greater** than the target's (admin is exempt), **and** the target must not be an Admin-group member — **no one** (not another admin, not the account itself) can delete an Admin member, so the root account can never be removed in one request. A manager cannot delete a manager or an admin. |
| Write a user's policy (group memberships / personal ceiling) | actor's tier must be **strictly greater** than the target's (admin exempt). A non-admin can never write their own policy. Additionally, an Admin member's membership list is **immutable**: any attempt to change an Admin member's group memberships is rejected — assigning the Admin group to anyone is forbidden, and removing it from an Admin member is forbidden — so no one, including the admin, can demote the root account. Identity (username/password) and personal-ceiling edits on an Admin member remain allowed. |
...
| **Logs** | View the audit trail (who did what, when, from where) | `can_view_audit_logs` (admin included); Admin and Manager groups carry it by default, User off |
...
4. **Global host ceiling** — the global active count must stay below
   ...
   admins included, and admin instances still count toward it.
...
Rejections carry a clear reason — a short message and a scope identifying the
rule that fired (for example the per-user ceiling or the host ceiling) — so the
dashboard can show exactly why a launch was refused.
...
- Login returns the user's effective context; the dashboard refreshes it from
  the server on every visit.
- The session token carries **only identity** (user id and expiry) — no role or
  permission claims. A stale token can never outlive a permission change because
  every request resolves the effective context fresh from the database.
- The session cookie is set at login and cleared at logout. Any user can change
  their own password.
...
- [System Architecture](architecture.md) — what the platform is, how sessions are created and connected
- [Remote Authentication](remote-auth.md) — how sessions are secured
- [OpenAPI specification](../../apps/api/security/openapi.json) — generated endpoint payloads and auth requirements
