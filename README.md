...
| --- | --- | --- | --- |
| **單一工作區靜態開銷** | Base OS 需佔用 **1.0 – 2.0 GB** 記憶體 | VNC 容器僅佔用 **~250 MB** 記憶體 | **記憶體節省 80%+** |
| **環境啟動速度** | 15-30 秒（等待系統和桌面啟動） | **< 10 秒**（容器啟動 + Traefik 路由熱加載） | **連線速度快 2+ 倍** |
...
| **記憶體讀寫效能** | 雙頁表轉譯（EPT/NPT），帶來 **5–15% 存取延遲與頻寬損耗** | 原生存取 (Near-Native)，共用 Host Page Cache，**零虛擬化開銷** | **100% 宿主機原生速度** |
| **磁碟讀寫效能** | 虛擬磁碟鏡像 (qcow2/raw) 轉譯層，帶來 **10–30% 的 I/O 吞吐量衰減** | Direct Bind Mount，直接存取宿主機檔案系統，**零 Pass-through 損耗** | **完全釋放 Disk/NVMe 潛能** |
