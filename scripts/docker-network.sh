...
if docker network inspect "$NETWORK_NAME" >/dev/null 2>&1; then
    echo "> Docker 網路 '$NETWORK_NAME' 已經存在，略過建立步驟。"
else
    echo "> 正在尋找可用的 /16 網段並建立 Docker 網路 '$NETWORK_NAME'..."
    ...
    if [ -z "$SELECTED_SUBNET" ]; then
        echo "> 警告：未能在 172.16-31 範圍找到未使用的 /16 網段，改由 Docker 自動指派..."
        docker network create \
            --driver bridge \
            --subnet 172.28.0.0/16 \
            "$NETWORK_NAME"
    else
        echo "> 自動選擇未使用的網段: $SELECTED_SUBNET"
        docker network create \
            --driver bridge \
