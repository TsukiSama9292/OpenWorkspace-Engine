...
4. **implement & tdd** — Before writing any code, load every ticket into the working Todo list (as R1…Rn, in ticket-number order) so the whole frontier is visible and tracked. Then work the frontier (tickets whose blockers are all done): dispatch each non-blocked ticket to its own subagent via the `task` tool, in **parallel**, one ticket per subagent. Each subagent implements exactly that single ticket following this skill — test-first at the pre-agreed seams (red → green, one slice at a time), typecheck and run that ticket's test file while working, and use the `codebase-design` vocabulary when designing module interfaces. Mark a ticket done only when its subagent returns green, then promote the tickets it unblocks. Blocked tickets stay pending in the Todo until their blockers land. Run the full suite once at the end.
