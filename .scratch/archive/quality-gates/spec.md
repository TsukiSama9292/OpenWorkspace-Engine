Status: completed — delivered across `.scratch/archive/quality-gates/issues/01-be-quality-gates.md`, `02-fe-quality-gates.md`, `03-int-end-to-end.md` (all closed). Code-review revision: the Status line was stale (feature already shipped in commit `8d65e1b`) and Implementation Decisions carried file paths / code snippets contrary to the to-spec convention — both corrected here.
...
- **Clippy hard gate**: extend the API's existing zero-warning check script with a Clippy step over all targets and all features, warnings promoted to errors, and make the API package's check script run the same gate so the two entry points cannot disagree. Base (non-restriction) Clippy lints only — default behaviour, no `pedantic`/`restriction` groups in the gate.
- **Restriction lints are invocation-only**: `too_many_lines` and `cognitive_complexity` are enabled by flags on the analysis command only, never by crate-level attributes — attributes would be promoted to errors by the hard gate's warnings-to-errors and silently re-introduce the hard gate the layered model defers. Thresholds (100 lines, 25 complexity) live in the API's Clippy configuration file.
- **Forbid unsafe**: the API crate roots declare the forbid-unsafe attribute so the compiler itself rejects any `unsafe` in our own code — stronger than a lint gate because it cannot be bypassed by script omission or allow-attributes. Two test helpers that mutated process environment variables (unsafe under the edition's rules) were refactored to build settings from an injected source instead; production logic unchanged.
- **ESLint flat config**: the web app gains a flat-config ESLint setup combining Svelte-aware and TypeScript-aware recommended rule sets plus Prettier compatibility, so `.svelte`/`.ts`/`.js` are all linted. Base recommended rules error (hard gate); `complexity` and `max-lines-per-function` warn (soft). Dev dependencies are scoped to the web package.
- **Web scripts**: the web package gains a lint script running the linter, its check script runs type-check plus lint in one command, and a separate analysis script surfaces the complexity warnings without failing the gate.
- **Root analysis group**: the root package forwards a consistent set of analysis entry points (Rust complexity, dependency-unsafe surface, code bloat, web complexity) to the app packages; all soft, non-blocking.
- **E2E package**: a new standalone workspace package using Playwright, registered in the workspace, with two scripts — one running the smoke project and one running the full project — plus a third running everything. The root forwards user-facing `test:e2e` / `test:e2e:full` commands to those package scripts.
- **Playwright config**: targets the running dev stack's public origin (the single proxy) so the full browser → proxy → API/instance path is exercised; a global setup pings the root and an API endpoint (any HTTP response, e.g. 401, proves the stack is up) and fails fast with an actionable "dev stack not running" message otherwise. Two projects — smoke and full — matched by test-file suffix; runs single-threaded (shared dev DB), captures traces on first retry, and never starts its own servers.
- **E2E cleanup**: the full suite deletes every instance it creates in teardown (routes are removed by the API on delete); smoke specs are read-only.
- **Turbo isolation**: the E2E scripts are deliberately named outside Turborepo's default task set so the regular suite never sweeps them in. Deviation from plan: the root Turborepo config **did** gain a declared `test` task with caching disabled — under the pinned turbo version an undeclared `test` task made the root test command error for every package, so it was pre-broken at the root; declaring it fixes the root test command (web and API suites run, E2E skipped).
- **Doc sync**: the agent guide and project docs get the new commands and gate model; this is part of the feature's Definition of Done.
...
- **Gate seams (Web)**: `pnpm check` (svelte-check + eslint) is clean on the current codebase; a synthetic error in a fixture demonstrates failure. Existing vitest suite (23 files / 290 tests) stays green.
