...
This is the mirror image of the host-port problem already fixed via `flock` (see `.scratch/archive/flock-port-lock/spec.md`): ports have a cross-process `flock` registry, subnets do not. The asymmetry is accidental; the same mechanism should cover both.
